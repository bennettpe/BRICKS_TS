...
DFHCOMMAREA_EDIT: PROCEDURE EXPOSE EIBCPOSN SCR. SET.
  ...
  COMMSCR. = ''
  COMMSCR.COMMAREA = ROW_COMM
  COMMSCR.NOTE = 'ID:' LEFT(ROW_ID, 2) ' Order:' LEFT(ROW_ORDER, 4) ' Frame:' LEFT(ROW_FRAME, 4)
  EXEC CICS CONVERSE MAP('EDITCOMM') MAPSET(SET.MAPSET) FROM(COMMSCR.) INTO(COMMSCR.) ERASE END-EXEC

  /* Only save on ENTER. */
  IF C2X(EIBAID) \= '7D' THEN
    RETURN
  
  IF ROW_COMM = COMMSCR.COMMAREA THEN
    RETURN
  ELSE
    SCR.MSGALT = 'Press ENTER to commit changes.'
  ROW_COMM = COMMSCR.COMMAREA
...
LINK_FILTER: PROCEDURE EXPOSE FRAME. LINK. SCR. SET.
  FILTERSCR. = ''
  FILTERSCR.FILTERM = ''
  EXEC CICS CONVERSE MAP('EDITLINKFILTER') MAPSET(SET.MAPSET) FROM(FILTERSCR.) INTO(FILTERSCR.) ERASE END-EXEC

  /* Only set on ENTER. */
  IF C2X(EIBAID) \= '7D' THEN
    RETURN

  SET.FILTER = UPPER(FILTERSCR.FILTERM)
  SET.PAGE = 1
  RETURN
...
DATA_EXPORT: PROCEDURE EXPOSE FRAME. LINK. SCR. SET.
  EXPORTSCR. = ''
  EXEC CICS CONVERSE MAP('EDITEXPORT') MAPSET(SET.MAPSET) FROM(EXPORTSCR.) INTO(EXPORTSCR.) ERASE END-EXEC
...
DATA_IMPORT: PROCEDURE EXPOSE FRAME. LINK. SCR. SET.
  IMPORTSCR. = ''
  EXEC CICS CONVERSE MAP('EDITIMPORT') MAPSET(SET.MAPSET) FROM(IMPORTSCR.) INTO(IMPORTSCR.) ERASE END-EXEC
